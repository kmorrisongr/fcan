...
# If you want to filter by difference from baseline, pass a diff_opts (see feats_diff)
...
fcu_wrap_gafs = function(data, group, min_feat_scale, iters, popsize, elite, pmutation, pcrossover, do_penalize, functions=rfGA){
...
# Calculate change over baseline for each subject using method, return a list of length num_subjects of data.frames
fcu_covb = function(fc, times, subjects, baseline=fcu_only_attr(fc, times[1], times, "times"), method=covb_simple){
	master_foch = list()

	for (s in 1:length(subjects)){
		s_foch = as.data.frame(matrix(nrow=length(times[-1]), ncol=ncol(fc)-(fcu_first_feat_col(fc)-1)))
		rownames(s_foch) = times[-1]
		colnames(s_foch) = colnames(fc)[-1]

		for (point in times[-1]){
			fc_time = fcu_only_attr(fc, point, times, "times")

			time_read = which(s == fc_time$subject)
			time_foch = vector("numeric")

			# If there is no entry for our subject at this time
			if (length(time_read) == 0){
				time_foch = NA

			} else {
				for (i in fcu_first_feat_col(fc_time):ncol(fc_time)){
					# This s indexing works implicitly because all subjects are present at baseline
					base_measure = baseline[s,i]
					measure = fc_time[time_read,i]

					# Change relative to baseline
					time_foch[i-(fcu_first_feat_col(fc)-1)] = method(base_measure, measure)
				}
			}

			s_foch[point,] = time_foch
		}

		master_foch[[s]] = s_foch
	}
